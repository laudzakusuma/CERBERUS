...
async function simpleAlertTest() {
    ...
    const testHash = ethers.keccak256(ethers.toUtf8Bytes('simple_test_' + Date.now()));
    const modelHash = ethers.keccak256(ethers.toUtf8Bytes("cerberus-v2"));
    
    // ULTRA MINIMAL PARAMETERS
    const params = [
        testHash,         // bytes32 _txHash
        wallet.address,   // address _flaggedAddress
        [],               // address[] _relatedAddresses (empty)
        0,                // uint8 _level (INFO = 0)
        1,                // uint8 _category (RUG_PULL = 1) 
        90,               // uint256 _confidenceScore
        90,               // uint256 _severityScore
        "Test",           // string _description (minimal)
    ...
    console.log('  Level: 0 (INFO)');
    console.log('  Category: 1 (RUG_PULL)');
    ...
    try {
        ...
        if (receipt.status === 1) {
            console.log('🎉 SUCCESS! Alert reported successfully');
